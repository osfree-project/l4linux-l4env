		 ...
		 struct pt_regs *regs, unsigned long mask)
{
	...
	err |= __put_user(current->thread.pfa, &sc->cr2);
	      ...
	      sigset_t *oldset,	struct pt_regs * regs)
{
	...
	printk("%s: delivering signal %ld to %d(%s), eip: %lx, eflags: %lx, "
	       "cs: %x, orig_eax=%lx\n",
	       __func__, sig, current->pid, current->comm, regs->eip,
	       regs->eflags, regs->xcs, regs->orig_eax);
#endif
	int ret;
	...
	printk("%s: setting stack frame (%lx): eip: %lx, eflags: %lx, cs: %x\n",
	       __func__, regs->esp, regs->eip, regs->eflags, regs->xcs);
#endif
 ...
 * mistake.
 */
void fastcall do_signal(struct pt_regs *regs)
{
	siginfo_t info;
	int signr;
