...
#define phys_to_virt_r(x) ((void *)__pa(x))
	/*
	 * Check whether this memory region would entirely overlap
	 * the vmalloc area.
	 */
	if (phys_to_virt_r(mb->start) >= VMALLOC_MIN) {
	 ...
	 * the vmalloc area.
	 */
	if (phys_to_virt_r(mb->start + mb->size) < phys_to_virt_r(mb->start) ||
	    phys_to_virt_r(mb->start + mb->size) > VMALLOC_MIN) {
