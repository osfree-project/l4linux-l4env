...
good_area:
	 ...
	 */
survive:
	fault = handle_mm_fault(mm, vma, addr & PAGE_MASK, (fsr & (1 << 1)) ? FAULT_FLAG_WRITE : 0);
...
do_page_fault(unsigned long addr, unsigned int fsr, struct pt_regs *regs)
{
	...
	if (fsr & PF_EUSER) __do_user_fault(tsk, addr, fsr, sig, code, regs);
	return -1;

no_context:
	printk("%s: nocontext for %s(%d) at %08lx\n", __func__, tsk->comm, tsk->pid, addr);
	__do_kernel_fault(mm, addr, fsr, regs);
	return -1;
}
