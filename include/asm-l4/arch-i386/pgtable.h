#ifndef __ASM_L4__ARCH_I386__PGTABLE_H__
#define __ASM_L4__ARCH_I386__PGTABLE_H__
...
#define _PAGE_BIT_MAPPED	9	/* Page mapped via IPC */
...
#define _PAGE_MAPPED	0x200	/* Page mapped via IPC */
...
static inline int pte_mapped(pte_t pte)		{ return (pte).pte_low & _PAGE_MAPPED; }
...
#define __not_in_l4__ptep_test_and_clear_dirty(vma, addr, ptep) ({			\
	int __ret = 0;							\
	if (pte_dirty(*(ptep)))						\
...
#define __not_in_l4__ptep_test_and_clear_young(vma, addr, ptep) ({			\
	int __ret = 0;							\
	if (pte_young(*(ptep)))						\
...
static inline pte_t ptep_get_and_clear_full_l4disabled(struct mm_struct *mm, unsigned long addr, pte_t *ptep, int full)
{
	pte_t pte;
	if (full) {
		/*
...
static inline void ptep_set_wrprotect_l4disabled(struct mm_struct *mm, unsigned long addr, pte_t *ptep)
{
	clear_bit(_PAGE_BIT_RW, &ptep->pte_low);
	pte_update(mm, addr, ptep);
}

/* for ptep_test_and_clear_dirty, ptep_test_and_clear_young,
 * ptep_set_wrprotect, ptep_mkdirty and ptep_get_and_clear_full
 * we just take the generic functions as we need the set_pte
 * call and the generic functions do it this way
 */
...
#endif /* __ASM_L4__ARCH_I386__PGTABLE_H__ */
